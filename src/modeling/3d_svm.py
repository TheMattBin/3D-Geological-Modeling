...
def voxel_model_SVM(df, gamma, dist, seabed_raster):
    """
    Build a 3D SVM geotechnical model and return grid and result matrices.
    """
    litoPoints = []
    ...
    soil_class = litoNp[:, 3]
    clf = svm.SVC(kernel='rbf', probability=True, gamma=gamma)
...
def main():
    ...
    gamma = 0.5
    dist = 100
    output_filename = f'myOutputSVM-PC-{datetime.now().strftime("%d-%m-%Y-%H-%M-%S")}.nc'
    df = pd.read_csv(csv_path)
    seabed_raster = rasterio.open(seabed_raster_path)
    a, b, c, d, e = voxel_model_SVM(df, gamma, dist, seabed_raster)
