...
def voxel_model_RF(coor, coor_trans, soil_class, dist, seabed_raster):
    """
    Build a 3D Random Forest geotechnical model and return grid and result matrices.
    """
    clf = RandomForestClassifier(45)
...
def main():
    # Parameters
    csv_path = 'CentralWater_20211230_pengchau.csv'
    seabed_raster_path = 'seabed_no_hk.tif'
    dist = 100
    output_filename = f'myOutputRF-PengChau-{datetime.now().strftime("%d-%m-%Y-%H-%M-%S")}.nc'
    df = pd.read_csv(csv_path)
    seabed_raster = rasterio.open(seabed_raster_path)
    litoNp, coor_trans, soil_class = point_cloud_trans(df, dist)
    a, b, c, d, e = voxel_model_RF(litoNp, coor_trans, soil_class, dist, seabed_raster)
