...
def acc_model(coor_trans, soil_class):
...
def voxel_model_RF(coor, coor_trans, soil_class, dist):
    # neighbours defined
    # we create an instance of Neighbours Classifier and fit the data.
...
def createVoxel(cellCols, cellRows, cellLays, litoMatrix, ProbMatrix):
    ...
    # Create NetCDF
    outDataSet = netCDF4.Dataset('myOutputRF-PengChau-{}.nc'.format(t.strftime('%d-%m-%Y-%H-%M-%S')), 'w', format = 'NETCDF4')
...
for i in [1, 100, 10000]:
    litoNp, coor_trans, soil_class = point_cloud_trans(df_pc, i)
    a,b,c,d,e = voxel_model_RF(litoNp,coor_trans,soil_class,i)
