...
def voxel_model_SVM(df, gamma, dist):
...
def createVoxel(cellCols, cellRows, cellLays, litoMatrix, ProbMatrix):
    ...
    # Create NetCDF
    outDataSet = netCDF4.Dataset('myOutputSVM-PC-{}.nc'.format(t.strftime('%d-%m-%Y-%H-%M-%S')), 'w', format = 'NETCDF4')
...
for i in [1, 100, 1000, 10000]:
    a,b,c,d,e = voxel_model_SVM(df_pc, 0.5, i)
