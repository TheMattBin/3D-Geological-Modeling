...
def voxel_model_GBC(coor, coor_trans, soil_class, ne, dist):
...
def createVoxel(cellCols, cellRows, cellLays, litoMatrix, ProbMatrix):
    ...
    # Create NetCDF
    outDataSet = netCDF4.Dataset('myOutputGBC-pengchau{}.nc'.format(t.strftime('%d-%m-%Y-%H-%M-%S')), 'w', format = 'NETCDF4')
...
for i in [100]:
    litoNp, coor_trans, soil_class = point_cloud_trans(df, i)
    a,b,c,d,e = voxel_model_GBC(litoNp,coor_trans,soil_class,400, i)
