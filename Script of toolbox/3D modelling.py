...
def knn3d(table, xmin, xmax, ymin, ymax, xresolution, yresolution, zresolution, ratio, seabed, file, name, n_neighbors):
    ...
    '''arcpy.analysis.SpatialJoin("pt_on_seabed", 'HK_Shape', 'ptinpoly', 'JOIN_ONE_TO_ONE', 'KEEP_COMMON', '#', 'INTERSECT')
    pt_land_arr = []
    pt_land = arcpy.da.SearchCursor('ptinpoly', ['SHAPE@XY', 'SHAPE@Z'])
    for row in pt_land:
        pt_land_arr.append([format(row[0][0], '.1f'), format(row[0][1], '.1f'), format(row[1], '.1f')])'''
    ...
    #  find points inside the land
    '''arcpy.analysis.SpatialJoin("pt_on_seabed", 'HK_Shape', 'ptinpoly', 'JOIN_ONE_TO_ONE', 'KEEP_COMMON', '#', 'INTERSECT')
    pt_land_arr = []
    pt_land = arcpy.da.SearchCursor('ptinpoly', ['SHAPE@XY', 'SHAPE@Z'])
    for row in pt_land:
        pt_land_arr.append([format(row[0][0], '.1f'), format(row[0][1], '.1f')])'''
    ...
    '''for lay in range(nLays):
        for row in range(nRows):
            for col in range(nCols):
                cellXYZ = [format(cellCols[col], '.1f'), format(cellRows[row], '.1f')]
                if cellXYZ in pt_land_arr:
                    litoMatrix[lay, row, col] = 10
                    ProbMatrix[lay, row, col] = ma.masked'''

    #arcpy.management.Delete('ptinpoly')
    #arcpy.management.Delete('ptinpoly')
